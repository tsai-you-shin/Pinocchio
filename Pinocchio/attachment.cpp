...
public:
    ...
    virtual pair<int, double> getnzWeights(int i, int j) const = 0;
...
public:
    ...
    pair<int, double> getnzWeights(int i, int j) const { return nzweights[i][j]; }
...
pair<int, double> Attachment::getnzWeights(int i, int j) const { return a->getnzWeights(i, j); }
